...
TEST(MemsetAsync, NullArguments) {
    EXPECT_EQ(cudaSuccess, cudaMemsetAsync(NULL, 0, 0, 0));

    /* Caught by Panoptes.  Disabling for now
    EXPECT_EQ(cudaErrorInvalidValue, cudaMemsetAsync(NULL, 0, 4, 0));
    cudaGetLastError(); */
}

TEST(MemsetAsync, MallocAfterMemset) {
    cudaError_t ret;
    void *ptr1, *ptr2;
    ...
    ret = cudaMemsetAsync(ptr1, 0, block, 0);
    ASSERT_EQ(cudaSuccess, ret);
...
TEST_P(MemsetAsyncValidity, Aligned) {
    ...
    for (size_t i = 0; i < param; i += 2) {
        ...
        cudaError_t ret;
        ret = cudaMemsetAsync(ptr    + range, i & 0x0, range);
        ASSERT_EQ(cudaSuccess, ret);
...
INSTANTIATE_TEST_CASE_P(MemsetAsyncValidityInst, MemsetAsyncValidity,
    ::testing::Range(1, 20));
