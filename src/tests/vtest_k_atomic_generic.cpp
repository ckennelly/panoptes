...
template<typename T>
class AtomicGenericFixture : public ::testing::Test {
public:
    typedef T _TypeParam;
protected:
    AtomicGenericFixture() { }
    ~AtomicGenericFixture() { }
};

TYPED_TEST_CASE(AtomicGenericFixture, AtomicTypes);

TYPED_TEST(AtomicGenericFixture, NoInitialization) {
    cudaError_t ret;
    ...
    const bool bret = launch_atomic_generic<TypeParam>(2, d, a, b);
    EXPECT_TRUE(bret);
...
TYPED_TEST(AtomicGenericFixture, MixInInitialized) {
    cudaError_t ret;
...
TYPED_TEST(AtomicGenericFixture, Initialized) {
    cudaError_t ret;
...
TYPED_TEST(AtomicGenericFixture, AddressInitialized) {
    cudaError_t ret;
 ...
 */
class AtomicGenericFixtureCAS : public ::testing::TestWithParam<unsigned> {
public:
    AtomicGenericFixtureCAS() { }
    ~AtomicGenericFixtureCAS() { }
};

TEST_P(AtomicGenericFixtureCAS, DoTest) {
    ...
    const uint32_t a_init = 0;
    if (!(p & 0x1)) {
        VALGRIND_MAKE_MEM_UNDEFINED(&a_init, sizeof(a_init));
    }
    ...
    const bool bret = launch_atomic_generic_cas(2, d, a, b, c);
    EXPECT_TRUE(bret);
...
INSTANTIATE_TEST_CASE_P(GenericCASTests, AtomicGenericFixtureCAS,
    ::testing::Range(0u, 8u));

TEST(GenericOverrun, DoTest) {
    const size_t N = 13;
    uint32_t * a;
    ...
    for (size_t i = 0; i < N; i++) {
        ret = launch_red_generic_overrun(a + i);
        EXPECT_EQ(cudaSuccess, ret);
    }
     ...
     */
    ret = launch_red_generic_overrun(a + (1 << 18));
