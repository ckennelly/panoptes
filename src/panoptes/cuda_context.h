...
#ifndef __PANOPTES__CUDA_CONTEXT_H__
#define __PANOPTES__CUDA_CONTEXT_H__
...
struct ptx_t;
class global_cuda_context;

class cuda_context : public context {
public:
    /**
    ...
    cuda_context(global_cuda_context * ctx, int device, unsigned int flags);
...
protected:
    friend class global_cuda_context;
    ...
    global_cuda_context * global() { return global_; }
    const global_cuda_context * global() const { return global_; }

    internal::modules_t * modules_;
private:
    global_cuda_context * const global_;
...
#endif // __PANOPTES__CUDA_CONTEXT_H__
