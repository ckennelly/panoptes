...
protected:
     ...
     *
     * must_free indicates that we should flag the block to Valgrind (if present)
     */
...
private:
     ...
     */
    BOOST_STATIC_ASSERT(sizeof(((chunk_aux_t *) 0)->allocations) * CHAR_BIT > lg_chunk_bytes);
    ...
    /**
     * We keep a single immutable handle as the default target for uninitialized blocks.
     */
